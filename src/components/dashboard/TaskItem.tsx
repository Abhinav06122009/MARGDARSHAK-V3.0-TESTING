...
const TaskItem: React.FC<TaskItemProps> = React.memo(({
  ...
  return (
    <motion.div
      ...
      className={`p-4 md:p-6 rounded-2xl border-2 ${getStatusBorderColor(task.status)} bg-black/20 backdrop-blur-md
    ...
    >
      <div className="flex items-start space-x-3 md:space-x-6">
        ...
        <motion.button
          ...
          className={`w-6 h-6 md:w-7 md:h-7 mt-1 rounded-lg border-2 flex items-center justify-center transition-all duration-200 flex-shrink-0 ${
            isSelected
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-white mb-2 text-lg md:text-2xl group-hover:text-emerald-300 transition-colors truncate">
            {task.title}
          </div>
          {task.description && (
            <p className="text-xs md:text-sm text-white/70 mb-4 max-w-prose line-clamp-2 md:line-clamp-none">
              {task.description}
            </p>
          )}
